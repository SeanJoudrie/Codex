...
for (const list of sources.awesome_lists) {
  const readme = await gh(`/repos/${list.repo}/readme`, { accept: 'application/vnd.github.raw', raw: true });
  if (!readme) { console.warn(`awesome list missing: ${list.repo}`); continue; }
...
console.log(`discover: ${candidates.length} new candidates`);
